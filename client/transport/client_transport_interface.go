...
type ClientTransport interface {
	...
	RegisterToolProvider(ctx context.Context, manualProvider server.Provider) ([]server.Tool, error)

	// DeregisterToolProvider removes a previously registered provider.
	DeregisterToolProvider(ctx context.Context, manualProvider server.Provider) error
	...
	CallTool(ctx context.Context, toolName string, arguments map[string]any, toolProvider server.Provider, l *string) (any, error)
}
