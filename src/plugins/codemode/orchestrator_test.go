...
func TestPlanAndGenerate_ExprRoundTrip(t *testing.T) {
	...
	if !strings.Contains(prompt, "EXPR CODEMODE CONTRACT") {
		t.Fatal("missing Expr contract")
	}
	if strings.Contains(prompt, "CodeMode Go") || strings.Contains(prompt, "__out") || strings.Contains(prompt, " := ") {
		t.Fatal("prompt contains Go-oriented CodeMode instructions")
	}
	if plan.Code != code {
		t.Fatalf("unexpected code: %s", plan.Code)
	}
}
...
func TestPlanAndGenerate_Errors(t *testing.T) {
	cases := []struct {
		...
		{"invalid Expr", `{"tools":["tool1"],"code":"let =","stream":false}`, nil, "snippet validation failed"},
	}
...
func TestValidateGeneratedPlan_Expr(t *testing.T) {
	...
	if err := validateGeneratedPlan(valid, []string{"tool1"}, []string{"tool1", "tool2"}); err != nil {
		t.Fatalf("valid plan rejected: %v", err)
	}
	bad := generatedPlan{Tools: []string{"unknown"}, Code: `codemode.CallTool("unknown", {})`}
	if err := validateGeneratedPlan(bad, []string{"unknown"}, []string{"tool1"}); err == nil {
		t.Fatal("expected unavailable-tool error")
	}
	missing := generatedPlan{Tools: nil, Code: `codemode.CallTool("tool1", {})`}
	if err := validateGeneratedPlan(missing, []string{"tool1"}, []string{"tool1"}); err == nil || !strings.Contains(err.Error(), "missing from tools list") {
		t.Fatalf("unexpected error: %v", err)
	}
	unused := generatedPlan{Tools: []string{"tool1", "tool2"}, Code: `codemode.CallTool("tool1", {})`}
	if err := validateGeneratedPlan(unused, []string{"tool1"}, []string{"tool1", "tool2"}); err == nil || !strings.Contains(err.Error(), "unused tool") {
		t.Fatalf("unexpected error: %v", err)
	}
}
...
func TestValidateGeneratedPlan_StreamFlag(t *testing.T) {
	...
	if err := validateGeneratedPlan(plan, []string{"tool1"}, []string{"tool1"}); err == nil || !strings.Contains(err.Error(), "stream flag") {
		t.Fatalf("unexpected error: %v", err)
	}
}
...
func TestRankToolSpecs(t *testing.T) {
	...
	if len(ranked) != 1 || ranked[0].Name != "memory.search" {
...
func TestExtractGeneratedToolNames(t *testing.T) {
	...
	if !reflect.DeepEqual(got, []string{"tool1", "tool2"}) {
...
func TestRenderUtcpToolsForPrompt(t *testing.T) {
	...
	for _, expected := range []string{"TOOL: test.tool", "DESCRIPTION: A test tool", "- arg1: string", "REQUIRED FIELDS:", "FULL INPUT SCHEMA (JSON):"} {
...
func TestCallTool_NoCandidateToolsSkipsModel(t *testing.T) {
	...
	if err != nil || needed || result != "" || calls != 0 {
		t.Fatalf("unexpected result: needed=%v result=%#v err=%v calls=%d", needed, result, err, calls)
	}
}

func TestCallTool_ExprExecution(t *testing.T) {
	client := &mockUTCP{searchToolsFn: func(string, int) ([]tools.Tool, error) {
		return []tools.Tool{{Name: "test.tool", Description: "execute test"}}, nil
	}}
	calls := 0
	mock := &mockModel{GenerateFunc: func(_ context.Context, prompt string) (any, error) {
		calls++
		if !strings.Contains(prompt, "TOOL: test.tool") {
			return nil, errors.New("tool schema missing")
		}
	...
	cm.executeFunc = func(_ context.Context, args CodeModeArgs) (CodeModeResult, error) {
		if args.Code != `codemode.CallTool("test.tool", {})` {
			t.Fatalf("unexpected Expr: %s", args.Code)
		}
		return CodeModeResult{Value: "ok"}, nil
	}
	needed, result, err := cm.CallTool(context.Background(), "execute test")
	if err != nil || !needed || calls != 1 {
		t.Fatalf("unexpected result: needed=%v result=%#v err=%v calls=%d", needed, result, err, calls)
	}
	if result.(CodeModeResult).Value != "ok" {
